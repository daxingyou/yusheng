		...
		try {
			...
			for (L1PcInstance tg : L1World.getInstance().getVisiblePlayer(pc)) {
				if (tg.getCurrentHp() == 0 && tg.isDead()) {
					tg.sendPackets(new S_SystemMessage("GM救了你一命。"));
					...
					tg.sendPackets(new S_SystemMessage("GM治疗了。"));
			...
			if (pc.getCurrentHp() == 0 && pc.isDead()) {
				pc.sendPackets(new S_SystemMessage("GM救了你一命。"));
				pc.broadcastPacket(new S_SkillSound(pc.getId(), 3944));
				pc.sendPackets(new S_SkillSound(pc.getId(), 3944));
				// 祝福された 复活スクロールと同じ效果
				pc.setTempID(objid);
				pc.sendPackets(new S_Message_YN(322, "")); // また复活したいですか？（Y/N）
			} else {
				pc.sendPackets(new S_SystemMessage("GM治疗了。"));
