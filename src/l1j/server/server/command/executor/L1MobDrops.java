	...
	public void execute(L1PcInstance pc, String cmdName, String arg) {
		...
		try {
			...
			for (L1Drop drop : mobDrops) {
				...
				if (item.getBless() == 1) {blessed = "";
				} else if (item.getBless() == 0) {blessed = "\\fR";
				} else {
					blessed = "\\fY";
				}
				double chance = drop.getChance() / (double) 10000;
				pc.sendPackets(new S_SystemMessage(blessed + "数量: "+ drop.getMin() + "~" + drop.getMax() + " 物品: "+ drop.getItemid() + " " + item.getName() + " 机率: "+chance + "%"));
			}
		} catch (Exception e) {
			pc.sendPackets(new S_SystemMessage("请输入 ." + cmdName + "[NPC编号]。"));
		} finally {
			SQLUtil.close(rs);
			SQLUtil.close(pstm);
			SQLUtil.close(con);
