...
public class C_Amount extends ClientBasePacket {
	...
	public C_Amount(byte[] decrypt, LineageClient _client) throws Exception {
		super(decrypt);
		int objectId = readD();
		int amount = readD();
		int c = readC();
		...
		final L1Object object = L1World.getInstance().findObject(objectId);
		...
		if (s1.equalsIgnoreCase("agapply") && npc != null) { // 竞卖入札场合
		...
		} else if (s1.equalsIgnoreCase("agsell") && npc != null) { // 家卖场合
			int houseId = Integer.valueOf(s2);
			AuctionBoardTable boardTable = new AuctionBoardTable();
			...
			if (board != null) {
				...
				house.setOnSale(true); // 竞卖中设定
				house.setPurchaseBasement(true); // 地下未购入设定
				HouseTable.getInstance().updateHouse(house); // DB书迂
			}
		} else {
