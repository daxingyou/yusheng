...
public class PetTable {
	...
	//获得宠物 
	public void storeNewPet(L1Pet pet, int objid, int itemobjid) {
		L1Pet l1pet = new L1Pet();
		l1pet.set_itemobjid(itemobjid);
		l1pet.set_objid(objid);
		//变更
	...
	public void UpdatePet(L1Pet pet) {
		Connection con = null;
		PreparedStatement pstm = null;
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
	...
	public static boolean isNameExists(String nameCaseInsensitive) {
		...
		try {
			...
			if (PetTypeTable.getInstance().isNameDefault(nameLower)) { // 名前重复
				return false;
			}
		} catch (SQLException e) {
			_log.error(e.getLocalizedMessage(), e);
