...
public class L1CastleLocation {
	...
	/**
	 * 战争（旗内）座标castle_id返
	 */
	public static int getCastleIdByArea(L1PcInstance pc) {
		return getCastleIdByArea(pc.getLocation());
	}
	...
	public static boolean checkInWarAreaIsNowWar(final L1Location loc){
		final int castleId = getCastleIdByArea(loc);
		if (castleId != 0){
	...
	public static int[] getWarArea(int castleId) {
		int[] loc = new int[5];
		if (castleId == KENT_CASTLE_ID) { // 城
			loc[0] = KENT_X1;
			loc[1] = KENT_X2;
			loc[2] = KENT_Y1;
			loc[3] = KENT_Y2;
			loc[4] = KENT_MAP;
		} else if (castleId == OT_CASTLE_ID) { // 森
			loc[0] = OT_X1;
			loc[1] = OT_X2;
			loc[2] = OT_Y1;
			loc[3] = OT_Y2;
			loc[4] = OT_MAP;
		} else if (castleId == WW_CASTLE_ID) { // 城
			loc[0] = WW_X1;
			loc[1] = WW_X2;
			loc[2] = WW_Y1;
			loc[3] = WW_Y2;
			loc[4] = WW_MAP;
		} else if (castleId == GIRAN_CASTLE_ID) { // 城
			loc[0] = GIRAN_X1;
			loc[1] = GIRAN_X2;
			loc[2] = GIRAN_Y1;
			loc[3] = GIRAN_Y2;
			loc[4] = GIRAN_MAP;
		} else if (castleId == HEINE_CASTLE_ID) { // 城
			loc[0] = HEINE_X1;
			loc[1] = HEINE_X2;
			loc[2] = HEINE_Y1;
			loc[3] = HEINE_Y2;
			loc[4] = HEINE_MAP;
		} else if (castleId == DOWA_CASTLE_ID) { // 城
			loc[0] = DOWA_X1;
			loc[1] = DOWA_X2;
			loc[2] = DOWA_Y1;
			loc[3] = DOWA_Y2;
			loc[4] = DOWA_MAP;
		} else if (castleId == ADEN_CASTLE_ID) { // 城
			loc[0] = ADEN_X1;
			loc[1] = ADEN_X2;
			loc[2] = ADEN_Y1;
			loc[3] = ADEN_Y2;
			loc[4] = ADEN_MAP;
		} else if (castleId == DIAD_CASTLE_ID) { // 要塞
			loc[0] = DIAD_X1;
			loc[1] = DIAD_X2;
			loc[2] = DIAD_Y1;
			loc[3] = DIAD_Y2;
	...
	public static int[] getCastleLoc(int castle_id) { // castle_id城内座标返
		int[] loc = new int[3];
		if (castle_id == KENT_CASTLE_ID) { // 城
			loc[0] = 32731;
			loc[1] = 32810;
			loc[2] = 15;
		} else if (castle_id == OT_CASTLE_ID) { // 森
			loc[0] = 32800;
			loc[1] = 32277;
			loc[2] = 4;
		} else if (castle_id == WW_CASTLE_ID) { // 城
			loc[0] = 32730;
			loc[1] = 32814;
			loc[2] = 29;
		} else if (castle_id == GIRAN_CASTLE_ID) { // 城
			loc[0] = 32724;
			loc[1] = 32827;
			loc[2] = 52;
		} else if (castle_id == HEINE_CASTLE_ID) { // 城
			loc[0] = 32568;
			loc[1] = 32855;
			loc[2] = 64;
		} else if (castle_id == DOWA_CASTLE_ID) { // 城
			loc[0] = 32853;
			loc[1] = 32810;
			loc[2] = 66;
		} else if (castle_id == ADEN_CASTLE_ID) { // 城
			loc[0] = 32892;
			loc[1] = 32572;
			loc[2] = 300;
		} else if (castle_id == DIAD_CASTLE_ID) { // 要塞
			loc[0] = 32733;
			loc[1] = 32985;
			loc[2] = 330;
		}
