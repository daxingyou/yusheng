...
public class L1Poison6 {
	...
	private static L1Poison6 _instance;

	public L1Poison6() {
	}

	public static L1Poison6 getInstance() {
		if (_instance == null) {
			_instance = new L1Poison6();
		}
		return _instance;
	}
		...
		public void run() {
			...
			if (_l1object instanceof L1PcInstance) {
				...
				if (_player.isDead() == false) {
					...
					_player.sendPackets(new S_Poison(_targetId, 2));
					_player.broadcastPacket(new S_Poison(_targetId, 2));
					_player.sendPackets(new S_Paralysis(2, _timesec,true));
			...
			if (_l1object instanceof L1MonsterInstance) {
				...
				if (_mob.isDead() == false) {
					...
					_mob.broadcastPacket(new S_Poison(_targetId, 2));
					_paralysis_timer = new ParalysisTimer(_l1object, _time);
					GeneralThreadPool.getInstance().execute(_paralysis_timer);
				}
			}
			...
			if (_l1object instanceof L1PetInstance) {
				...
				if (_pet.isDead() == false) {
					...
					_pet.broadcastPacket(new S_Poison(_targetId, 2));
					_paralysis_timer = new ParalysisTimer(_l1object, _time);
					GeneralThreadPool.getInstance().execute(_paralysis_timer);
				}
			}
			...
			if (_l1object instanceof L1SummonInstance) {
				...
				if (_summon.isDead() == false) {
					...
					_summon.broadcastPacket(new S_Poison(_targetId, 2));
					_paralysis_timer = new ParalysisTimer(_l1object, _time);
					GeneralThreadPool.getInstance().execute(_paralysis_timer);
				}
			}
	...
	class ParalysisTimer extends Thread {
		...
		public void run() {
			...
			if (_l1object instanceof L1PcInstance) {
				_player = (L1PcInstance) _l1object;
//				if (_player.isDead() == false) {
					_player.sendPackets(new S_Paralysis(2, 0, false));
					CurePoison(_l1object);
//			}
			}
	...
	public boolean handleCommands(L1Object l1object, int type, int time,
		...
		if (l1object instanceof L1PcInstance) {
			...
			_targetId = _player.getId();
			if (_player.get_poisonStatus6() != 0) {
				return false;
			}
			_player.set_poisonStatus6(type);
			_player.sendPackets(new S_Poison(_targetId, 2));
			_player.broadcastPacket(new S_Poison(_targetId, 2));
		}
			...
			_targetId = _mob.getId();
			if (_mob.get_poisonStatus6() != 0) {
				return false;
			}
			_mob.set_poisonStatus6(type);
			_mob.broadcastPacket(new S_Poison(_targetId, 2));
		}
		else if (l1object instanceof L1PetInstance) {
			_pet = (L1PetInstance) l1object;
			_targetId = _pet.getId();
			if (_pet.get_poisonStatus6() != 0) {
				return false;
			}
			_pet.set_poisonStatus6(type);
			_pet.broadcastPacket(new S_Poison(_targetId, 2));
		}
		else if (l1object instanceof L1SummonInstance) {
			_summon = (L1SummonInstance) l1object;
			_targetId = _summon.getId();
			if (_summon.get_poisonStatus6() != 0) {
				return false;
			}
			_summon.set_poisonStatus6(type);
			_summon.broadcastPacket(new S_Poison(_targetId, 2));
		}
	...
	public void CurePoison(L1Object l1object) {
		if (l1object instanceof L1PcInstance) {
			...
			if (_player.get_poisonStatus6() == 1) {
				_isPoisonTimerDelete = true;
			}
			else if (_player.get_poisonStatus6() == 4) {
				_isParalysisPoisonTimerDelete = true;
			}
			else {
				return;
			}
			_player.set_poisonStatus6(0);
			_player.sendPackets(new S_Poison(_targetId, 0));
			_player.broadcastPacket(new S_Poison(_targetId, 0));
		}
		else if (l1object instanceof L1MonsterInstance) {
			_mob = (L1MonsterInstance) l1object;
			_targetId = _mob.getId();

			if (_mob.get_poisonStatus6() == 1)
			{
				_isPoisonTimerDelete = true;
			}else if (_mob.get_poisonStatus6() == 4) {
				_isParalysisPoisonTimerDelete = true;
			}else {
				return;
			}
			_mob.set_poisonStatus6(0);
			_mob.broadcastPacket(new S_Poison(_targetId, 0));
		}
		else if (l1object instanceof L1PetInstance) {
			_pet = (L1PetInstance) l1object;
			_targetId = _pet.getId();

			if (_pet.get_poisonStatus6() == 1)
			{
				_isPoisonTimerDelete = true;
			}
			else if (_pet.get_poisonStatus6() == 4) {
				_isParalysisPoisonTimerDelete = true;
			}
			else {
				return;
			}
			_pet.set_poisonStatus6(0);
			_pet.broadcastPacket(new S_Poison(_targetId, 0));
		}
		else if (l1object instanceof L1SummonInstance) {
			_summon = (L1SummonInstance) l1object;
			_targetId = _summon.getId();

			if (_summon.get_poisonStatus6() == 1)
			{
				_isPoisonTimerDelete = true;
			}
			else if (_summon.get_poisonStatus6() == 4) {
				_isParalysisPoisonTimerDelete = true;
			}
			else {
				return;
			}
			_summon.set_poisonStatus6(0);
			_summon.broadcastPacket(new S_Poison(_targetId, 0));
		}
		else if (l1object instanceof L1TowerInstance) {
			_tower = (L1TowerInstance) l1object;
			_targetId = _tower.getId();
			_tower.set_poisonStatus6(0);
		}
		else {
			_npc = (L1NpcInstance) l1object;
			_targetId = _npc.getId();
			_npc.set_poisonStatus6(0);
		}
	}
}
