...
public class L1Poison4 {
	...
	private static L1Poison4 _instance;

	public L1Poison4() {
	}

	public static L1Poison4 getInstance() {
		if (_instance == null) {
			_instance = new L1Poison4();
		}
		return _instance;
	}
		...
		public void run() {
			...
			if (_l1object instanceof L1PcInstance) {
				...
				if (_player.isDead() == false) {
					...
					_player.sendPackets(new S_Paralysis(4, _timesec,true));
	...
	class ParalysisTimer extends Thread {
		...
		public void run() {
			...
			if (_l1object instanceof L1PcInstance) {
				_player = (L1PcInstance) _l1object;
//				if (_player.isDead() == false) {
					_player.sendPackets(new S_Paralysis(4,0, false));
					CurePoison(_l1object);
//				}
			}
	...
	public void CurePoison(L1Object l1object) {
		if (l1object instanceof L1PcInstance) {
			...
			if (_player.get_poisonStatus4() == 1) {
				_isPoisonTimerDelete = true;
			}
			else if (_player.get_poisonStatus4() == 4) {
				_isParalysisPoisonTimerDelete = true;
			}
			else {
				return;
			}
			_player.set_poisonStatus4(0);
		}
		else if (l1object instanceof L1MonsterInstance) {
			_mob = (L1MonsterInstance) l1object;
			_targetId = _mob.getId();

			if (_mob.get_poisonStatus4() == 1)
			{
				_isPoisonTimerDelete = true;
			}
			else if (_mob.get_poisonStatus4() == 4) {
				_isParalysisPoisonTimerDelete = true;
			}
			else {
				return;
			}
			_mob.set_poisonStatus4(0);
		}
		else if (l1object instanceof L1PetInstance) {
			_pet = (L1PetInstance) l1object;
			_targetId = _pet.getId();

			if (_pet.get_poisonStatus4() == 1)
			{
				_isPoisonTimerDelete = true;
			}
			else if (_pet.get_poisonStatus4() == 4) {
				_isParalysisPoisonTimerDelete = true;
			}
			else {
				return;
			}
			_pet.set_poisonStatus4(0);
		}
		else if (l1object instanceof L1SummonInstance) {
			_summon = (L1SummonInstance) l1object;
			_targetId = _summon.getId();

			if (_summon.get_poisonStatus4() == 1)
			{
				_isPoisonTimerDelete = true;
			}
			else if (_summon.get_poisonStatus4() == 4) {
				_isParalysisPoisonTimerDelete = true;
			}
			else {
				return;
			}
			_summon.set_poisonStatus4(0);
		}
		else if (l1object instanceof L1TowerInstance) {
			_tower = (L1TowerInstance) l1object;
			_targetId = _tower.getId();
			_tower.set_poisonStatus4(0);
		}
		else {
			_npc = (L1NpcInstance) l1object;
			_targetId = _npc.getId();
			_npc.set_poisonStatus4(0);
		}
	}
}
