...
public class L1WeaponSkill {
	...
	private int _effectTarget; // 对像 0:相手 1:自分
	...
	public static double getBaphometStaffDamage(L1PcInstance pc, L1Character cha) {
		double dmg = 0;
		final L1WilliamWeaponSkill tempWeaponSkill = WeaponSkill.getInstance().getTemplate(124);
		if (tempWeaponSkill == null){
			return dmg;
		}
		Random random = new Random();
		int chance = random.nextInt(100) + 1;
		if (tempWeaponSkill.getProbability() >= chance) {
			int locx = cha.getX();
			int locy = cha.getY();
			int sp = pc.getSp();
			int intel = pc.getInt();
