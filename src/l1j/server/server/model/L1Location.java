...
public class L1Location extends Point {
	...
	public void setPowerMap(int mapId) {
		_map = L1WorldMap.getInstance().getPowerMap((short) mapId);
	}
	...
	/**
	 * 引数Location对、移动可能范围Location返。
	 * 场合、城、内Location返却。
	 * 
	 * @param baseLocation
	 *            范围元Location
	...
	public static L1Location randomLocation(L1Location baseLocation, int min,
		...
		if (min > max) {
			throw new IllegalArgumentException("min > max引数无效");
		}
		if (max <= 0) {
		...
		while (true) {
			...
			if (isRandomTeleport) { // 场合
				if (L1CastleLocation.checkInAllWarArea(newX, newY, mapId)) { // 城
					continue;
				}
