...
/*		if (isDead()) {
	...
	public void receiveDamage(L1Character attacker, int damage) { // 攻击ＨＰ减使用
		if (_castle_id == 0) { // 初期设定良场所
			_castle_id = L1CastleLocation.getCastleId(getX(), getY(),
					getMapId());
		}
		if (_castle_id > 0
				&& WarTimeController.getInstance().isNowWar(_castle_id)) { // 战争时间内
...
/*			boolean existDefenseClan = false;
			...
			for (L1War war : L1World.getInstance().getWarList()) {
				if (_castle_id == war.GetCastleId()) { // 今居城战争
					isProclamation = war.CheckClanInWar(pc.getClanname());
					break;
				}
			}*/
/*			if (existDefenseClan == true && isProclamation == false) { // 城主居、布告场合
				return;
			}*/
