...
public class EchoServerTimer extends TimerTask {
	...
	public void start() {
		try {
			...
			/*if (Config.RESTART_LOGIN > 0) {
				_log.info("监听端口重置作业启动 间隔时间:" + Config.RESTART_LOGIN + "分钟。");
	...
	public void startEcho() {
		try {
			...
			for (String ports : portList) {
				final String[] portInts = ports.split(",");
				final int key = Integer.parseInt(portInts[0]);
				final int language = Integer.parseInt(portInts[1]);
				ServerExecutor echoServer = new ServerExecutor(key,language);
				if (echoServer != null) {
	 ...
	 * @param port
	 */
	public void startPort(final int key,final int language) {
		try {
			ServerExecutor echoServer = _echoList.get(new Integer(key));
			if (echoServer == null) {
				echoServer = new ServerExecutor(key,language);
				_echoList.put(new Integer(key), echoServer);
				echoServer.stsrtEcho();
