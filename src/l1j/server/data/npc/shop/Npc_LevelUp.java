...
//	}
		if (cmd.equalsIgnoreCase("c")) {// 我想调整我的属性
			final L1ItemInstance item = pc.getInventory().findItemId(l1j.william.New_Id.Item_AJ_27);
			if (item != null) {
				pc.clear_uplevelList();
				info = showInfo(pc, 2);
			...
			final L1ItemInstance item = pc.getInventory().findItemId(l1j.william.New_Id.Item_AJ_27);
			if (item != null) {
				int elixirStats = pc.get_uplevelList(0);
				if (elixirStats > 0) {
			...
			if (item != null) {
				...
				pc.addBaseStr((byte)-baseStr);
				pc.addBaseDex((byte)-baseDex);
				pc.addBaseCon((byte)-baseCon);
				pc.addBaseWis((byte)-baseWis);
				pc.addBaseInt((byte)-baseInt);
				pc.addBaseCha((byte)-baseCha);
				
				int originalStr = pc.get_uplevelList(1);
				int originalDex = pc.get_uplevelList(2);
				int originalCon = pc.get_uplevelList(3);
				int originalWis = pc.get_uplevelList(4);
				int originalInt = pc.get_uplevelList(5);
				int originalCha = pc.get_uplevelList(6);
				
				int addStr = pc.get_uplevelList(7);
				int addDex = pc.get_uplevelList(8);
				int addCon = pc.get_uplevelList(9);
				int addWis = pc.get_uplevelList(10);
				int addInt = pc.get_uplevelList(11);
				int addCha = pc.get_uplevelList(12);
				...
				// 变更原始素质设定
				pc.setOriginalStr(pc.get_newPcOriginal()[0]);
				pc.setOriginalDex(pc.get_newPcOriginal()[1]);
				pc.setOriginalCon(pc.get_newPcOriginal()[2]);
				pc.setOriginalWis(pc.get_newPcOriginal()[3]);
				pc.setOriginalInt(pc.get_newPcOriginal()[4]);
				pc.setOriginalCha(pc.get_newPcOriginal()[5]);
...
/*				if (pc.getOriginalAc() > 0) {
				...
				try {
					...
					WriteLogTxt.Recording(
							"回忆蜡烛","玩家#"+pc.getName()+"#玩家objid：<"+pc.getId()+">等级<"
							+pc.getLevel()
					        +">#回忆完毕，初始力量#"+pc.getOriginalStr()
									+"#,初始敏捷#"+pc.getOriginalDex()
									+"#,初始体质#"+pc.getOriginalCon()
									+"#,初始智力#"+pc.getOriginalInt()
									+"#,初始精神#"+pc.getOriginalWis()
									+"#,初始魅力#"+pc.getOriginalCha()
									+"#,力量#"+pc.getBaseStr()
									+"#,敏捷#"+pc.getBaseDex()
									+"#,体质#"+pc.getBaseCon()
									+"#,智力#"+pc.getBaseInt()
		...
		if (info != null) {
			int xmode = pc.get_uplevelList(13);
			switch (xmode) {
			case 0:// 0:升级点数
	...
	public static void setHpMp(final L1PcInstance pc) {
		for (int i = 0; i < pc.getLevel(); i++) {
			final short randomHp = 
				CalcStat.calcStatHp(pc.getType(), pc.getBaseMaxHp(), pc.getBaseCon());
			
			final short randomMp = 
				CalcStat.calcStatMp(pc.getType(), pc.getBaseMaxMp(), pc.getBaseWis());
	...
	public static String[] showInfoX(final L1PcInstance pc, final int mode) {
		...
		switch (xmode) {
		case 0:// 0:升级点
			max = new int[]{Config.BONUS_STATS1, Config.BONUS_STATS1,Config.BONUS_STATS1, Config.BONUS_STATS1, Config.BONUS_STATS1, Config.BONUS_STATS1};//ConfigAlt.POWER;
			break;
			
		case 1:// 1:万能药
			max = new int[]{Config.BONUS_STATS3,Config.BONUS_STATS3, Config.BONUS_STATS3, Config.BONUS_STATS3, Config.BONUS_STATS3, Config.BONUS_STATS3};//ConfigAlt.POWERMEDICINE;
			break;
		...
		// 更新剩余点数
		pc.add_levelList(0, elixirStats);
	 ...
	 * 初始化点选
	 * @param pc 执行人物
	 * @param isBonus 0:升级点 1:万能药 2:人物出生点数
	 * @return
	 */
	public static String[] showInfo(final L1PcInstance pc, final int mode) {
		...
		case 1:// 1:万能药
			...
			originalStr = pc.get_uplevelList(1);
			originalDex = pc.get_uplevelList(2);
			originalCon = pc.get_uplevelList(3);
			originalWis = pc.get_uplevelList(4);
			originalInt = pc.get_uplevelList(5);
			originalCha = pc.get_uplevelList(6);

			// 万能药使用次数重选(13-1)
			pc.add_levelList(13, 1);
			break;
		...
		case 0:// 0:升级点
			...
			int addStrS = pc.get_uplevelList(7);
			int addDexS = pc.get_uplevelList(8);
			int addConS = pc.get_uplevelList(9);
			int addWisS = pc.get_uplevelList(10);
			int addIntS = pc.get_uplevelList(11);
			int addChaS = pc.get_uplevelList(12);
			
			pc.add_levelList(1, originalStr + addStrS);// 力量 (原始) + 上一阶段(人物出生点数)所决定的点数
			pc.add_levelList(2, originalDex + addDexS);// 敏捷 (原始) + 上一阶段(人物出生点数)所决定的点数
			pc.add_levelList(3, originalCon + addConS);// 体质 (原始) + 上一阶段(人物出生点数)所决定的点数
			pc.add_levelList(4, originalWis + addWisS);// 精神 (原始) + 上一阶段(人物出生点数)所决定的点数
			pc.add_levelList(5, originalInt + addIntS);// 智力 (原始) + 上一阶段(人物出生点数)所决定的点数
			pc.add_levelList(6, originalCha + addChaS);// 魅力 (原始) + 上一阶段(人物出生点数)所决定的点数
			
			// 暂存人物原始素质改变
			pc.set_newPcOriginal(
					new int[]{
							originalStr + addStrS,// 力量 (原始) + 上一阶段(人物出生点数)所决定的点数
							originalDex + addDexS,// 敏捷 (原始) + 上一阶段(人物出生点数)所决定的点数
							originalCon + addConS,// 体质 (原始) + 上一阶段(人物出生点数)所决定的点数
							originalWis + addWisS,// 精神 (原始) + 上一阶段(人物出生点数)所决定的点数
							originalInt + addIntS,// 智力 (原始) + 上一阶段(人物出生点数)所决定的点数
		...
		case 1:// 1:万能药
			...
			int addStrR = pc.get_uplevelList(7);
			int addDexR = pc.get_uplevelList(8);
			int addConR = pc.get_uplevelList(9);
			int addWisR = pc.get_uplevelList(10);
			int addIntR = pc.get_uplevelList(11);
			int addChaR = pc.get_uplevelList(12);
			
			pc.add_levelList(1, originalStr + addStrR);// 力量 (原始) + 上一阶段(升级属性提升值)所决定的点数
			pc.add_levelList(2, originalDex + addDexR);// 敏捷 (原始) + 上一阶段(升级属性提升值)所决定的点数
			pc.add_levelList(3, originalCon + addConR);// 体质 (原始) + 上一阶段(升级属性提升值)所决定的点数
			pc.add_levelList(4, originalWis + addWisR);// 精神 (原始) + 上一阶段(升级属性提升值)所决定的点数
			pc.add_levelList(5, originalInt + addIntR);// 智力 (原始) + 上一阶段(升级属性提升值)所决定的点数
			pc.add_levelList(6, originalCha + addChaR);// 魅力 (原始) + 上一阶段(升级属性提升值)所决定的点数
			break;
			
		case 2:// 2:人物出生点数
			pc.add_levelList(1, originalStr);// 力量 (原始)
			pc.add_levelList(2, originalDex);// 敏捷 (原始)
			pc.add_levelList(3, originalCon);// 体质 (原始)
			pc.add_levelList(4, originalWis);// 精神 (原始)
			pc.add_levelList(5, originalInt);// 智力 (原始)
			pc.add_levelList(6, originalCha);// 魅力 (原始)
			break;
		}
	...
	private static String[] info(final L1PcInstance pc) {
		String[] info = null;
		int p1 = pc.get_uplevelList(0);// 剩余可用点数
		
		int p2 = pc.get_uplevelList(1);// 力量 (原始)
		int p3 = pc.get_uplevelList(7);// 力量 +-
		
		int p4 = pc.get_uplevelList(2);// 敏捷 (原始)
		int p5 = pc.get_uplevelList(8);// 敏捷 +-
		
		int p6 = pc.get_uplevelList(3);// 体质 (原始)
		int p7 = pc.get_uplevelList(9);// 体质 +-
		
		int p8 = pc.get_uplevelList(4);// 精神 (原始)
		int p9 = pc.get_uplevelList(10);// 精神 +-
		
		int p10 = pc.get_uplevelList(5);// 智力 (原始)
		int p11 = pc.get_uplevelList(11);// 智力 +-
		
		int p12 = pc.get_uplevelList(6);// 魅力 (原始)
		int p13 = pc.get_uplevelList(12);// 魅力 +-
