...
//////////特定地点,使用物品触发怪物
	public static void forRequestItemUSe(LineageClient client,L1ItemInstance itemInstance){
	...
	//////////移动触发怪物
	public static void forCRequestMoveChar(LineageClient client){
		L1PcInstance user = client.getActiveChar();
		int aTempData[] = null;
		int id = 0;
		int iDamage=0;
...
////// 以下是兑换活动
					...
					for(int i=0;i<aData8.size();i++){
					  ...
					  if( equipeitem.getItemId()== ((Integer) aTempData8.get(0)).intValue() ){
						  ...
						  if( ((Integer) aTempData8.get(1)).intValue()<= iExp) {
							  ...
							  upgradeWeapon(equipeitem.getId(),((Integer) aTempData8.get(2)).intValue(), ItemTable.getInstance().getTemplate(((Integer) aTempData8.get(2)).intValue()).getName() );
	...
	public static void addWeaponExp(L1PcInstance l1pcinstance ,long exp){
...
//END	
   ...
   private static Object getArray(String s,String sToken,int iType){
	...
	private static void mobspawn(LineageClient clientthread, int i) {
