...
public class Reward {
	...
	public  void getItem(L1PcInstance pc) {
		...
		for (int i=0; i <array.size(); i++) {
			...
			if (pc.getLevel() >= ((Integer) data.get(0)).intValue() && 
				...
				pc.getQuest().get_step(((Integer) data.get(11)).intValue()) != ((Integer) data.get(12)).intValue()) { // 等级符合
					...
					for (int j = 0; j < materials.length; j++) {
						...
						if (item != null) {
							if ((String) data.get(13) != null && isGet == false) {
								pc.sendPackets(new S_SystemMessage((String) data.get(13)));//讯息
								isGet = true;
							}
							...
							if (pc.getInventory().checkAddItem(item, (counts[j])) == L1Inventory.OK) {
								...
								item.setDropObjId(pc.getId());//设置该物品掉落时的对象
				...
				if (((Integer) data.get(6)).intValue() != 0 && pc.isDragonKnight()) {// 龙骑士
					boolean isGet = false;
					int[] materials = (int[]) data.get(8);
					int[] counts = (int[]) data.get(9);
					int[] enchantLevel = (int[]) data.get(10);
				...
				if (((Integer) data.get(7)).intValue() != 0 && pc.isIllusionist()) {// 幻术师
					boolean isGet = false;
					int[] materials = (int[]) data.get(8);
					int[] counts = (int[]) data.get(9);
					int[] enchantLevel = (int[]) data.get(10);
        ...
        try {
          ...
          if ( rset!=null)
            while (rset.next()) {
            	...
            	arraylist.add(6, new Integer(rset.getInt("give_DragonKnight")));//龙骑士
            	arraylist.add(7, new Integer(rset.getInt("give_Illusionist")));//幻术师
    ...
    private  Object getArray(String s, String sToken, int iType) {
